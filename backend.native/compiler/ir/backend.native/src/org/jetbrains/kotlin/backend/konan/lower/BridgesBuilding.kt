    ...
    private fun buildWorkerBridges(declaration: IrDeclaration): List<IrFunction> {
        ...
        declaration.transformChildrenVoid(object: IrElementTransformerVoid() {
            ...
            override fun visitCall(expression: IrCall): IrExpression {
                ...
                val argType = jobDescriptor.valueParameters[0].type
                val returnType = jobDescriptor.returnType!!
                val argConversion = symbols.getTypeConversion(nullableAnyType, argType)
                val returnConversion = symbols.getTypeConversion(returnType, nullableAnyType)
                if (argConversion == null && returnConversion == null)
                    return expression
                val arg = jobDescriptor.valueParameters[0]
                val overriddenJobDescriptor = jobDescriptor.newCopyBuilder()
                ...
                val bridge = context.buildBridge(
                        ...
                        descriptor   = OverriddenFunctionDescriptor(jobDescriptor, overriddenJobDescriptor),
