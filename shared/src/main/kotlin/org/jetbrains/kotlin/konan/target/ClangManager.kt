...
class TargetClang(private val clangManager: ClangManager, private val target: KonanTarget) {
    ...
    fun clang(vararg userArgs: String) = clangManager.targetClangCmd(target) + userArgs.asList()

    fun clangXX(vararg userArgs: String) = clangManager.targetClangXXCmd(target) + userArgs.asList()
}
